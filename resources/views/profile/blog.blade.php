<x-app-layout>
    <div class="text-center">
        <p class="font-extrabold text-2xl my-3">My Blog</p>
    </div>
    <main class="min-w-full flex justify-center">
        @if (session('status'))
            <div class="mt-4 p-4 bg-green-100 text-green-700 rounded w-full">
                {{ session('status') }}
            </div>
        @endif
            ...
            <div class="flex w-[1380px] flex-wrap gap-9">
                @foreach ($blogs as $blog)
                    <div class="w-[435px] object-cover flex flex-col gap-5">
                        <img src="{{ asset('storage/' . $blog->thumbnail) }}" class="w-full h-[300px] rounded-2xl">
